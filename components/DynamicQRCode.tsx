...
export class DynamicQRCode extends Component<DynamicQRCodeProps, DynamicQRCodeState> {
  ...
  componentDidMount() {
    this.buildFragmentsFromProps();
  }

  buildFragmentsFromProps = () => {
    const { value, capacity = 175, hideControls = true, walletID, protocol = 'auto' } = this.props;
    try {
      this.fragments =
        protocol === 'BBQR'
          ? this.buildRenderableBBQRFragments()
          : encodeUR(value, capacity, walletID ?? null, protocol === 'URv2' ? 'URv2' : undefined);
      this.setState(
        {
          index: 0,
